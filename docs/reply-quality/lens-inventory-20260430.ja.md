...
| `safe_connection` | 会話の流れを作る時に、同じ役割・同じ約束レベルの文だけをつなげているか | `block_rhythm_flow` の補助観点 |
| response_weight calibration | buyer の文量・温度・質問数に対して、返信の重さが合っているか | `conversation_flow_naturalness` / Gold 35 配下 |
| voice ownership | FAQ や窓口案内ではなく、この場の担当者が返している文に見えるか | 観察中。強めすぎると過約束に接続するため soft lens |
| certainty calibration | 情報量に応じて、断定・留保・条件付き回答の強さが合っているか | `promise_consistency` 近接の観察項目 |
| burden alignment | buyer に不要な判断・材料選別を押し戻していないか | 観察中。secret / 過剰材料要求に接続した時だけ hard guard 側で見る |
