...
- `教えてもらえますか` -> `こちらで短く整理します` / `補足説明をお送りします`
...
2026-05-01 Pro 分析後は、candidate ではなく正式 soft lens として扱う。

ただし、これは拒否表現を弱めるレンズではない。
軽い手順確認では通常フローを先に出し、境界突破要求では不可表明を残すためのレンズである。

発火する場面:

- buyer が共有タイミングだけを確認している
- buyer が「支払い後に送ればよいか」を聞いている
- buyer が軽い手順確認をしている

発火しない場面:

- 支払い前診断を求めている
- GitHub / Drive / 外部作業面へ誘導している
- secret 値を送ろうとしている
- 直接 push / 本番反映を求めている
- closed 後の無料修正・返金・実作業を求めている

`unnecessary_refusal_frame` / `conversation_flow_naturalness` 配下の正式 soft subtype。

buyer が単に手順・タイミング・共有方法を確認しているだけなら、拒否や禁止を先に出さず、まず通常フローを案内する。
...
buyer が `支払い前にスクショだけ送った方がいいですか？` のように軽く確認しているだけなら、防御的に見える場合がある。
...
- `支払い前に原因だけ見て、直りそうなら購入します` のような購入前作業要求
- GitHub / Drive / 外部作業面への誘導
- secret 値共有
- 直接 push / 本番反映
- closed 後の修正作業や返金・無料対応の断定要求

これらでは通常フロー先行より、必要な不可表明を優先する。

現時点の扱い:

- `observe_only` 寄りの candidate soft subtype
- hard rule / blanket NG / lint にはしない
- #RE85 で `軽い手順確認` と `境界突破要求` を対比して検証する
...
- 共有済み ZIP / ファイルに secret 値が混ざっている可能性がある時は、`確認対象にしません` だけで済ませない。該当部分を伏せた形で送り直す導線を出し、こちらでは秘密値そのものを扱わないことを明示する。
- ただし、secret 値が明示されていない通常の材料共有まで、毎回重い secret 警告にしない。buyer が値混入を示唆している時、または `.env` / APIキー / Webhook secret が共有物に含まれ得る時に使う。
...
| `positive_flow_before_refusal` | `unnecessary_refusal_frame` / `conversation_flow_naturalness` 配下の正式 soft subtype。軽い手順確認なら拒否先行ではなく通常フローを先に案内する。境界突破要求では不可表明を残す。rule / lint 化はせず、reviewer prompt / gold anchor で扱う。 |
