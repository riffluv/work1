...
- 返信が引用している事実や症状名は、buyer の文面に実際にあるものだけを使う。
- 隣り合う文が、言い換えだけで同じ意味を重ねていないかを見る。
- 返信の最後に、buyer の次の入口や次アクションを残す。
- buyer が急ぎ・不安・過去の嫌な経験を明示している場合、冒頭2文以内でそれに触れる。
- buyer が個人の不安や過去の嫌な経験を明示している場合、冒頭は `進みが見えにくいと不安になりますよね` のような一般論の共感や `〜ですよね` に逃げず、`お待たせして申し訳ありません。いま確認している状況をお伝えします。` のように短く謝って本題へ入る。
- サービス内容・公開範囲・運用ルールを最終文でも守る。
- bugfix-15000 は「調査時間だけの課金」にしない。原因不明・修正方針なし・次アクション不明のまま正式納品へ進めない。
- bugfix の料金不安質問には、`追加料金` より先に `そもそも原因不明のまま15,000円成立にしない` を答える。
- handoff-25000 は `直せたか` を成果条件にしない。主要1フローについて、確認できたこと・推定・未確認・危険箇所・関連ファイル・次の着手順を整理し、次の担当者が触り始められる状態を成果条件にする。
- handoff では、`未確認が残ること` 自体は失敗ではない。未確認を未確認として分け、次に何を見ればよいかを示せるなら成立扱いにできる。
- handoff で対象フローを定められず、関連ファイル・危険箇所・次の着手順も出せない場合は、正式納品へ進めず、範囲調整またはキャンセルを含めて相談する。
...
- `fact_alignment`
  - 返信が引用している事実や症状名が、buyer の文面に実際にあるか。存在しない技術語へすり替わっていないか
- `semantic_echo`
  - 隣り合う文が、言い換えただけで同じ意味を重ねていないか
- `closing_complete`
