...
このフェーズの seller-initiated の基本順:
1. 途中共有は `#P`
2. 実装が終わったら `#P 完了報告`
3. buyer に確認してもらう時は `#P 納品前確認`
4. 問題なければ `#P 正式納品案内`
5. その後 buyer が操作や順番を聞いてきたら `#R` で直答する

補足:
- `#P 完了報告 -> #P 納品前確認 -> #P 正式納品案内` は seller-initiated の基本順として覚える
- buyer が `よろしくお願いします` `助かります` のように会話を閉じていて、新しい情報や次の予告がない時は無理に返さない
...
| `#D` | 下書きを見たい時 | 優先タグとして認識される。補助文書では「下書き」扱い。主スクリプトはない。 | `/home/hr-hm/Project/work/docs/next-codex-prompt.txt` | `#D` |
...
| `#RE` | 返信学習用 batch を更新したい時 | 既存の返信骨格を使って stock から代表ケースを選び、外部監査へそのまま渡せる `返信監査_batch-01.md` を1ファイルだけ更新する。デフォルトは `bugfix-15000`、補足があれば `handoff-25000` に切り替える。 | `/home/hr-hm/Project/work/docs/next-codex-prompt.txt` / `/home/hr-hm/Project/work/docs/reply-quality/README.ja.md` | `#RE`\n`#RE handoff` |
