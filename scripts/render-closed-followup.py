...
def build_response_decision_plan(source: dict, scenario: str, contract: dict) -> dict:
    ...
    if scenario == "repeat_handoff_request":
        ...
        direct_answer_line = "前回の修正が戻ったのか別の変更かは、まず今回の状況を見てから切り分けます。"
    elif scenario == "new_flow_plus_discount":
        blocking_missing_facts = ["target_flow_and_note"]
        direct_answer_line = "別のフローも含めて見られるかは、まず今回見たい範囲を確認してからお返しします。"
        response_order = ["opening", "direct_answer", "answer_detail", "ask", "next_action"]
            ...
            "answer_map": [
                {
                    "question_id": "q1",
                    "disposition": "answer_after_check",
                    "answer_brief": "今回の範囲に入るかは、まず前回の続きとして同じ原因で見られるかを確認します。そのうえでお返しします。",
                    "hold_reason": "トークルームは閉じているため、追加の1点でも前回と同じ話かを先に見たいです。",
                    "revisit_trigger": "追加で出ている箇所を受領したあとに、今回の範囲で見られるかをお返しします。",
                },
            ],
            "ask_map": [
                {
            ...
            "answer_map": [
                {
                    ...
                    "hold_reason": "前回のトークルームは閉じているため、同じ話かどうかで案内が変わる前にまず今回の状況を見たいです。",
            ...
            "answer_map": [
                {
                    "question_id": "q1",
                    "disposition": "answer_after_check",
                    "answer_brief": "どこを触ったあとに戻ったかを見てから、今回どう進めるかをお伝えします。",
...
def current_focus_line(case: dict) -> str | None:
    ...
    if scenario in {"new_issue_repeat_client", "repeat_bugfix_price_check"}:
        return "いまの症状が分かれば、見積りの入口をこちらで整えます。"
    if scenario == "same_ticket_scope_question":
    ...
    if scenario == "generic_closed":
        return "トークルームは閉じているため、今回のご相談が前回の続きとして見られるか確認します。"
    return None
...
def draft_opening_anchor(case: dict) -> str:
    ...
    if scenario == "same_ticket_scope_question":
        return "今回の範囲に入るか気になっているとのこと、確認しました。"
    if scenario == "repeat_handoff_request":
        return "前回のメモが役に立ったとのこと、ありがとうございます。"
    if scenario == "memo_addendum_request":
...
def draft_body_paragraphs(case: dict) -> list[str]:
    ...
    if scenario in {"refund_request", "price_complaint", "same_symptom_recur", "similar_but_not_same", "self_edit_regression"}:
